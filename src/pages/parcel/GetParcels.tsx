...
export default function GetParcels() {
  ...
  const handleCancel = async (id: string) => {
    try {
      await cancelParcel(id).unwrap(); // backend decides if allowed or rejected
      alert("Parcel cancelled successfully"); // show success message
    } catch (err: any) {
      alert(err?.data?.message || "Failed to cancel parcel"); // show backend error
    }
  };
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {parcels.map((parcel: IParcel) => (
        ...
        >
            ...
            {/*  <p><strong>Sender:</strong> {parcel.senderId?.name}</p> 
          ...
          {/* Always show cancel button, frontend does not decide anything */}
          <button
